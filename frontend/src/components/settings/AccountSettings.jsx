...
import { ArrowLeft, Lock, Save, UserPlus, Users, Shield, ShieldOff, Trash2, KeyRound } from 'lucide-react';
...
export default function AccountSettings() {
  ...
  const [newUser, setNewUser] = useState({ username: '', full_name: '', email: '', role: 'technician', password: '' });
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      setShowCreate(false);
      setNewUser({ username: '', full_name: '', email: '', role: 'technician', password: '' });
      fetchUsers();
    } catch (err) {
  ...
  const handleDialogConfirm = async () => {
    ...
    try {
      if (dialog.type === 'delete') {
        ...
        if (!resetPw || resetPw.length < 6) { toast.error('Password must be at least 6 characters'); setLoading(false); return; }
        await axios.put(`${API_URL}/auth/users/${dialog.user.id}/reset-password`, { password: resetPw });
        ...
        toast.success(dialog.type === 'enable' ? 'User enabled' : 'User disabled');
      }
      fetchUsers();
      setDialog(null);
    } catch (err) {
          ...
          <CardContent>
            <form onSubmit={handlePasswordChange} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input type="password" value={pwForm.confirmPassword} required minLength={6}
                  onChange={e => setPwForm({ ...pwForm, confirmPassword: e.target.value })} />
              </div>
              <Button type="submit" disabled={loading} className="w-full">
                <Save className="h-4 w-4 mr-2" />{loading ? 'Updating...' : 'Update Password'}
              </Button>
            </form>
          </CardContent>
        </Card>
                      ...
                      {!isSelf && (
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" title="Reset password"
                            onClick={() => { setDialog({ type: 'reset', user }); setResetPw(''); }}>
                            <KeyRound className="h-4 w-4 text-blue-600" />
                          </Button>
                          {user.is_active ? (
                            <Button variant="ghost" size="icon" title="Disable"
                              onClick={() => setDialog({ type: 'disable', user })}>
                              <ShieldOff className="h-4 w-4 text-orange-600" />
                            </Button>
                          ) : (
                            <Button variant="ghost" size="icon" title="Enable"
                              onClick={() => setDialog({ type: 'enable', user })}>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input type="password" value={newUser.password} minLength={6}
          ...
          <AlertDialogHeader>
            <AlertDialogTitle>
              {dialog?.type === 'delete' ? 'Delete User' :
               ...
               dialog?.type === 'enable' ? 'Enable User' : 'Disable User'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {dialog?.type === 'delete' ? `Delete "${dialog.user.full_name}"? This cannot be undone.` :
               dialog?.type === 'reset' ? `Set a new password for "${dialog?.user?.full_name}":` :
               dialog?.type === 'enable' ? `Enable "${dialog.user.full_name}"?` :
               `Disable "${dialog?.user?.full_name}"?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          {dialog?.type === 'reset' && (
            <Input type="password" placeholder="New password (min 6 chars)" value={resetPw}
              onChange={e => setResetPw(e.target.value)} className="mt-2" />
          )}
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDialogConfirm} disabled={loading}
              className={dialog?.type === 'delete' ? 'bg-red-600 hover:bg-red-700' :
                         dialog?.type === 'disable' ? 'bg-orange-600 hover:bg-orange-700' : ''}>
